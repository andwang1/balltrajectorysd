                ...
                for (auto it = ea.pop().begin(); it != ea.pop().end(); ++it) {
                    if (index_indiv % Params::stat::period_saving_individual_in_population == 0) {
                        ...
                        for (size_t index_start_row_obs = offset;
                            ...
                            std::vector<std::vector<uint8_t>> row_observation;

                            for (size_t index = 0;
                                 index < row_observation_float_red.size();
                                 ++index) {
                                row_observation.push_back({row_observation_red[index],
                                                           row_observation_green[index],
                                                           row_observation_blue[index]});
                            }


                            rgb_image.push_back(row_observation);
                        }
                        save_png_image(
                ...
                for (auto it = ea.pop().begin(); it != ea.pop().end(); ++it) {
                    if (index_indiv % Params::stat::period_saving_individual_in_population == 0) {
                        ...
                        for (size_t index_start_row_obs = offset;
                            ...
                            std::vector<uint8_t> row_observation;
                            for (auto item_row: row_observation_float) {
                                row_observation.push_back(floor(255. * item_row));
                            }
                            grayscale_image.push_back(row_observation);
                        }
                        save_png_image(
