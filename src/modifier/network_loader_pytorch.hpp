...
public:
        ...
        while (_continue && (epoch < TParams::ae::nb_epochs)) {
                ...
                for (int i{0}; i < std::get<2>(tup).size(); ++i)
                {
                    ...
                    if (TParams::ae::full_loss)
                    {
                        if (TParams::ae::loss_function == TParams::ae::loss::L2)
                            {loss_tensor += torch::sum(torch::abs(torch::pow(traj[i] - reconstruction_tensor[index], 2) / (2 * torch::exp(decoder_logvar[index])) + 0.5 * (decoder_logvar[index] + _log_2_pi)));}
                        else if (TParams::ae::loss_function == TParams::ae::loss::L1)
                            {loss_tensor += torch::sum(torch::abs(torch::abs(traj[i] - reconstruction_tensor[index]) / (2 * torch::exp(decoder_logvar[index])) + 0.5 * (decoder_logvar[index] + _log_2_pi)));}
                        else if (TParams::ae::loss_function == TParams::ae::loss::SmoothL1)
                            {loss_tensor += torch::sum(torch::abs(torch::smooth_l1_loss(reconstruction_tensor[index], traj[i], 0) / (2 * torch::exp(decoder_logvar[index])) + 0.5 * (decoder_logvar[index] + _log_2_pi)));}
                    }
                    else
                    {
                        if (TParams::ae::loss_function == TParams::ae::loss::L2)
            ...
            if (!full_train) {
                ...
                if ((previous_avg.array() - previous_avg[0]).mean() > -1e-4 && epoch > TParams::ae::min_num_epochs &&
                    this->get_avg_recon_loss(train_phen, train_traj, tr_is_traj) < init_tr_recon_loss + 1e-4)
                        {_continue = false;}
            }
              ...
              MatrixXf_rm &decoder_var) 
    {
        ...
        for (int i{0}; i < boundaries.size(); ++i)
        {
            ...
            {
                if (TParams::ae::loss_function == TParams::ae::loss::L2)
                    {recon_loss_unreduced[i] = torch::abs(torch::pow(traj_tensor[i] - reconstruction_tensor[index], 2) / (2 * torch::exp(decoder_logvar[index])) + 0.5 * (decoder_logvar[index] + _log_2_pi));}
                else if (TParams::ae::loss_function == TParams::ae::loss::L1)
                    {recon_loss_unreduced[i] = torch::abs(torch::abs(traj_tensor[i] - reconstruction_tensor[index]) / (2 * torch::exp(decoder_logvar[index])) + 0.5 * (decoder_logvar[index] + _log_2_pi));}
                else if (TParams::ae::loss_function == TParams::ae::loss::SmoothL1)
                    {recon_loss_unreduced[i] = torch::abs(torch::smooth_l1_loss(reconstruction_tensor[index], traj_tensor[i], 0) / (2 * torch::exp(decoder_logvar[index])) + 0.5 * (decoder_logvar[index] + _log_2_pi));}
