...
def plot_avg_diversity_grid(path, generate_images=True, save_path=None):
    os.chdir(path)
    ...
    # plot grid
    plt.grid(which="both")
    sns.heatmap(div_grids, vmin=0, vmax=1, linewidths=0.2, fmt=".0f")
    ...
    # fig.colorbar(color)
    plt.title(f"Avg. Diversity - BinsTransversed / TotalBins - Gen {GEN_NUMBER}")
    plt.xlabel("X")
    plt.ylabel("Y")

    ax1 = fig.add_subplot()
    textbox = f"Avg. Score: {np.round(np.sum(div_grids), 2)} of {max_diversity}"
    ...
    if save_path:
        os.chdir(save_path)
    os.chdir(path)
    plt.savefig(f"diversity_avg_{GEN_NUMBER}.png")
    plt.savefig(f"diversity_avg_{GEN_NUMBER}.pdf")
    plt.close()

if __name__ == "__main__":
    plot_avg_diversity_grid(
